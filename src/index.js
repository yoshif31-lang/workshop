  ...
  async fetch(request, env) {
    ...
    if (request.method === "GET") {
      ...
      <head>
          ...
          <title>GENESYS-Core v1.0.4</title>
          <style>
      ...
      <body>
          <div id="header">
              <span>GENESYS-Core v1.0.4 [REPAIRED]</span>
              <button id="audioBtn" class="audio-toggle">Audio: OFF</button>
          </div>
          <div id="characters"><div class="char">凪</div><div class="char">刻</div><div class="char">響</div><div class="char">零</div></div>
          <div id="chat-box"></div>
          <div id="input-area">
          ...
          <script>
              ...
              async function send() {
                  ...
                  try {
                      const res = await fetch("/", { method: "POST", body: JSON.stringify({ query: text }) });
                      const data = await res.json();
                      const reply = data.answer || "通信エラー";
                      box.innerHTML += '<div class="msg"><b>Orbit:</b> ' + reply + '</div>';
                      box.scrollTop = box.scrollHeight;
                      speak(reply);
                  } catch(e) { box.innerHTML += '<div class="msg" style="color:red">ERROR</div>'; }
